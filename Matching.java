...
public class Matching {
  public static void main(String[] args) {
    ...
    // Predicates:
    Predicate<CD> eq2015 = cd -> cd.getYear().compareTo(Year.of(2015)) == 0;
    Predicate<CD> gt2015 = cd -> cd.getYear().compareTo(Year.of(2015)) > 0;

    boolean noneEQ2015 = CD.cdList.stream().noneMatch(eq2015);     // (4) true
    out.println("No CD issued in 2015: " + noneEQ2015);

    boolean allGT2015 = CD.cdList.stream().allMatch(gt2015);       // (5) true
    out.println("All CDs issued after 2015: " + allGT2015);

    boolean noneNotGT2015 = CD.cdList.stream().noneMatch(gt2015.negate()); //(6)
    out.println("No CDs issued in or before 2015: " + noneNotGT2015);    // true
