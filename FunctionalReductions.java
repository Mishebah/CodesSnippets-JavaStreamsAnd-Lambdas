...
// Two-arguments reduce() method:
  {
    ...
    for (CD cd : CD.cdList) {              // (2) Iterate over the list.
      int numOfTracks = cd.getNoOfTracks();// (3) Get the next value.
...
// Three-arguments reduce() method:
    ...
    //  .stream()                                          // (18a)
        ...
        .reduce(Integer.valueOf(0),                        // (19) Initial value
                (sum, cd) -> sum + cd.getNoOfTracks(),     // (20) Accumulator
...
//      .stream()                                          // (23a)
        ...
        .reduce(0,
               (sum, cd) -> {                              // (24) Accumulator
                 Integer noOfTracks = cd.getNoOfTracks();
    ...
    // Compare by CD title.
    Comparator<CD> cmpByTitle = Comparator.comparing(CD::getTitle); // (26)
    ...
    //  .max(cmpByTitle);                                  // (29c)
    optMaxJazzCD.map(CD::getTitle).ifPresent(out::println);// Keep on Erasing
  }
}