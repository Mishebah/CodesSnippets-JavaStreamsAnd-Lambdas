...
int totalNumOfTracks = CD.cdList
    .stream()                                         // (1) Stream<CD>
    .mapToInt(CD::getNoOfTracks)                      // (2) IntStream
    .sum();                                           // (3) 42
System.out.println(totalNumOfTracks);
}

{
List<CD> cdList1 = Arrays.asList(CD.cd0, CD.cd1);
List<CD> cdList2 = Arrays.asList(CD.cd2, CD.cd3, CD.cd4);
int totalNumOfTracks =
    Stream.of(cdList1, cdList2)                       // (1) Stream<List<CD>>
          .flatMapToInt(                              // (2)
              lst -> lst.stream()                     // (3) Stream<CD>
                        .mapToInt(CD::getNoOfTracks)) // (4) IntStream
                                                      // (5) Stream<IntStream>,
...
//         .map(n -> Arrays.asList(n, n*n))        // IntUnaryOperator expected. Compile-time error!
...
IntStream.rangeClosed(1, 3)                        // (1) IntStream
         .mapToObj(n -> Arrays.asList(n, n*n))     // (2) Stream<List<Integer>>
         .forEach(p -> System.out.print(p + " ")); // [1, 1] [2, 4] [3, 9]
System.out.println();

IntStream.rangeClosed(1, 3)                        // (3) IntStream
         .boxed()                                  // (4) Stream<Integer>
         .map(n -> Arrays.asList(n, n*n))          // (5) Stream<List<Integer>>
         .forEach(p -> System.out.print(p + " ")); // [1, 1] [2, 4] [3, 9]
System.out.println();
}
...
List<String> subListTitles = IntStream
    ...
    .mapToObj(i -> CD.cdList.get(i))                // (2) Stream<CD>
    .map(CD::getTitle)                              // (3) Stream<String>
    .collect(Collectors.toList());      // (4) [Lambda Dancing, Keep on Erasing]
System.out.println(subListTitles);
}

{
  List<Integer> intList = Arrays.asList(30, 15, 45, 10, 55);
  int from = 2, to = 4;
