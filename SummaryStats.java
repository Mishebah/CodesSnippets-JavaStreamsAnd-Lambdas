...
{ // Average number of tracks on a CD.
    ...
    if (CD.cdList.size() != 0) {
      int sumTracks = 0;
      for (CD cd: CD.cdList) {
        int numOfTracks = cd.getNoOfTracks();
...
{
//Summary statistics
IntSummaryStatistics stats1 = Arrays.asList(CD.cd0, CD.cd1).stream()
    .mapToInt(CD::getNoOfTracks)
    .summaryStatistics();
...
stats1.accept(CD.cd2.getNoOfTracks());  // Add the value 8.
System.out.println(stats1);
//IntSummaryStatistics{count=3, sum=24, min=6, average=8.000000, max=10}

IntSummaryStatistics stats2 = Arrays.asList(CD.cd3, CD.cd4).stream()
    .mapToInt(CD::getNoOfTracks)
    .summaryStatistics();
